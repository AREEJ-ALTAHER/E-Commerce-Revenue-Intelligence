view: event_session_funnel {
  derived_table: {
    sql:
      SELECT
        session_id,
        MIN(CASE WHEN event_type = 'Product' THEN created_at ELSE NULL END) AS event1_time,
        MIN(CASE WHEN event_type = 'Cart' THEN created_at ELSE NULL END) AS event2_time,
        MIN(CASE WHEN event_type = 'Purchase' THEN created_at ELSE NULL END) AS event3_time
